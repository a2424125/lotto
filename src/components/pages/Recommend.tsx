...
const Recommend: React.FC<RecommendProps> = ({
  ...
  const gradeInfo: { [key: string]: any } = {
    "1": {
      ...
      prize: "약 20억원",
      strategy: `${actualLatestRound}~${actualOldestRound}회차 (${totalRounds}개) 빅데이터 AI 완벽 분석`, // ✅ 실제 회차 범위 표시
      emoji: "👑",
      color: "#059669",
    },
    "2": {
  ...
  }, [totalRounds, roundRange]); // ✅ roundRange 변경시 재실행
  ...
  const generateFallbackStrategies = (
    ...
    for (let i = 0; i < 5; i++) {
      ...
      strategies.push({
        ...
        analysisData: {
          dataRange: `${actualLatestRound}~${actualOldestRound}회차 (${totalRounds}개)`, // ✅ 실제 범위 표시
          method: "기본 분석",
